...
func (m Mash) ExpectedBoilLoss() float64 {
	if m.boilOffVolume == 0 {
		m.boilOffVolume = DEFAULT_BOILOFF_PER_HOUR
	}
	if m.boilTime == 0 {
		m.boilTime = DEFAULT_BOIL_TIME
	}

	return (m.boilTime/60)*m.boilOffVolume
}
